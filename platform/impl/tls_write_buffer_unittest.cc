...
TEST(TlsWriteBufferTest, CheckBasicFunctionality) {
  auto buffer = std::make_unique<TlsWriteBuffer>();
  constexpr size_t write_size = TlsWriteBuffer::kBufferSizeBytes / 2;
  std::vector<uint8_t> write_buffer(write_size);
  std::fill(write_buffer.begin(), write_buffer.end(), uint8_t{1});

  EXPECT_TRUE(buffer->Push(ByteView(write_buffer.data(), write_size)));
  ...
  // written.
  EXPECT_FALSE(buffer->Push(ByteView(write_buffer.data(), write_size)));
  EXPECT_FALSE(buffer->Push(ByteView(write_buffer.data(), 1)));
}

TEST(TlsWriteBufferTest, TestWrapAround) {
  auto buffer = std::make_unique<TlsWriteBuffer>();
  constexpr size_t buffer_size = TlsWriteBuffer::kBufferSizeBytes;
  std::vector<uint8_t> write_buffer_data(buffer_size);
  std::fill(write_buffer_data.begin(), write_buffer_data.end(), uint8_t{1});
  ByteView write_buffer(write_buffer_data.data(), buffer_size);
  ...
  std::fill(write_buffer_data.begin(), write_buffer_data.end(), uint8_t{2});
  ...
  std::fill(write_buffer_data.begin(), write_buffer_data.end(), uint8_t{3});

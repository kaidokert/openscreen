...
class UdpSocketIntegrationTest : public ::testing::Test {
 public:
  void SetUp() override {
    // PlatformClientWin handles WSAStartup/Cleanup and starts the network loop
    // thread.
    PlatformClientWin::Create(std::chrono::milliseconds(50));
    task_runner_ = &PlatformClientWin::GetInstance()->GetTaskRunner();
  }

  void TearDown() override { PlatformClientWin::ShutDown(); }
...
class IntegrationClient : public UdpSocket::Client {
 public:
  void OnBound(UdpSocket* socket) override { bound_.store(true); }
...
TEST_F(UdpSocketIntegrationTest, SendAndReceive) {
  ...
  auto server_socket_result =
      UdpSocket::Create(*task_runner_, &server_client, server_addr);
  ...
  auto client_socket_result =
      UdpSocket::Create(*task_runner_, &client_client, {{127, 0, 0, 1}, 0});
  ...
  for (int i = 0; i < 200; ++i) {
    if (server_client.received_count_ > 0)
      break;
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }

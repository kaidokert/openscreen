...
#define WASP_LOG_STREAM(level_enum)                                      \
  ::openscreen::internal::LogMessage(::openscreen::LogLevel::level_enum, \
                                     __FILE__, __LINE__)

#define WASP_LAZY_STREAM(condition, stream) \
  !(condition) ? (void)0 : ::openscreen::internal::Voidify() & (stream)

#define WASP_LOG_INFO \
  WASP_LAZY_STREAM(WASP_LOG_IS_ON(kInfo), WASP_LOG_STREAM(kInfo))
#define WASP_LOG_WARN \
  WASP_LAZY_STREAM(WASP_LOG_IS_ON(kWarning), WASP_LOG_STREAM(kWarning))
#define WASP_LOG_ERROR \
  WASP_LAZY_STREAM(WASP_LOG_IS_ON(kError), WASP_LOG_STREAM(kError))
#define WASP_LOG_FATAL \
  WASP_LAZY_STREAM(WASP_LOG_IS_ON(kFatal), WASP_LOG_STREAM(kFatal))

#define WASP_CHECK(condition)      \
  WASP_LOG_IF(FATAL, !(condition)) \
      << "WASP_CHECK(" << #condition << ") failed: "

#define WASP_LOG_IF(level, condition) !(condition) ? (void)0 : WASP_LOG_##level
...
int main() {
  ...
  // This will hit the WASP_CHECK, but the goal is to make the macro itself
  // compile. obs.GetInstanceName("test");
  return 0;
}

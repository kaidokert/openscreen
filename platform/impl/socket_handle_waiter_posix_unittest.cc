...
TEST(SocketHandleWaiterBaseTest,
  ...
  // Socket handle2 should still be subscribed.
  EXPECT_CALL(
      waiter,
      AwaitSocketsReady(
          testing::Truly([&](const std::vector<
                             TestingSocketHandleWaiter::HandleWithSubscription>&
                                 hws_list) {
            return hws_list.size() == 1 &&
                   hws_list[0].ready_handle.handle.get() == handle2 &&
                   hws_list[0].subscription->subscriber == &subscriber2;
          }),
          _))
      .WillOnce(
          Return(ByMove(std::vector<TestingSocketHandleWaiter::ReadyHandle>{
      ...
      AwaitSocketsReady(
          testing::Truly([&](const std::vector<
                             TestingSocketHandleWaiter::HandleWithSubscription>&
                                 hws_list) {
            return hws_list.size() == 1 &&
                   hws_list[0].ready_handle.handle.get() == handle0 &&
                   hws_list[0].subscription->flags == subscribed_flags;
          }),
          _))
      .WillOnce(
          Return(ByMove(std::vector<TestingSocketHandleWaiter::ReadyHandle>{
  ...
  EXPECT_CALL(
      ...
      AwaitSocketsReady(
          testing::Truly([&](const std::vector<
            ...
            return hws_list.size() == 1 &&
                   hws_list[0].ready_handle.handle.get() == handle0 &&
                   hws_list[0].subscription->subscriber == &subscriber1 &&
                   hws_list[0].subscription->flags ==

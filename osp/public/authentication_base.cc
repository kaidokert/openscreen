...
std::vector<uint8_t> AuthenticationBase::ComputePublicValue(
  ...
  // Create a new EC_KEY object using the specified ellipic curve.
...
std::array<uint8_t, 64> AuthenticationBase::ComputeSharedKey(
  ...
  // Compute and save the shared secret using ECDM.
  ...
  // Hash the shared screct and provided password and use the final result as
  // the shared secret.
  SHA512_CTX sha512;
  SHA512_Init(&sha512);

...
ErrorOr<SOCKET> CreateNonBlockingUdpSocket(int domain) {
  ...
  if (sock == INVALID_SOCKET) {
    return Error(Error::Code::kInitializationFailure, std::to_string(WSAGetLastError()));
  }
...
ErrorOr<std::unique_ptr<UdpSocket>> UdpSocket::Create(
  ...
  int domain = (local_endpoint.address.version() == UdpSocket::Version::kV6) ? AF_INET6 : AF_INET;
    ...
    if (local_endpoint_.port == 0 && handle_.handle != INVALID_SOCKET) {
        ...
        if (getsockname(handle_.handle, (sockaddr*)&addr, &len) == 0) {
            if (addr.ss_family == AF_INET) {
                ...
                local_endpoint_.address = IPAddress(IPAddress::Version::kV4, (uint8_t*)&sin->sin_addr);
            } else if (addr.ss_family == AF_INET6) {
                sockaddr_in6* sin6 = (sockaddr_in6*)&addr;
                local_endpoint_.port = ntohs(sin6->sin6_port);
                local_endpoint_.address = IPAddress(IPAddress::Version::kV6, (uint8_t*)&sin6->sin6_addr);
            }
        }
    }
...
void UdpSocketWin::Bind() {
    ...
    BOOL opt = TRUE;
    setsockopt(handle_.handle, SOL_SOCKET, SO_REUSEADDR, (const char*)&opt, sizeof(opt));
    ...
    if (client_) {
        task_runner_.PostTask([weak_this = weak_factory_.GetWeakPtr()] {
            if (auto* self = weak_this.get()) {
                if (self->client_) self->client_->OnBound(self);
            }
        });
    }
}

void UdpSocketWin::SetMulticastOutboundInterface(NetworkInterfaceIndex ifindex) {
    if (handle_.handle == INVALID_SOCKET) return;
    ...
    if (IsIPv4()) {
        ...
        setsockopt(handle_.handle, IPPROTO_IPV6, IPV6_MULTICAST_IF, (const char*)&index, sizeof(index));
    }
}
...
void UdpSocketWin::JoinMulticastGroup(const IPAddress& address,
    ...
    if (IsIPv4()) {
        ...
        setsockopt(handle_.handle, IPPROTO_IP, IP_ADD_MEMBERSHIP, (const char*)&mreq, sizeof(mreq));
    } else {
        struct ipv6_mreq mreq;
        address.CopyToV6((uint8_t*)&mreq.ipv6mr_multiaddr);
        mreq.ipv6mr_interface = static_cast<unsigned int>(ifindex);
        setsockopt(handle_.handle, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, (const char*)&mreq, sizeof(mreq));
    }
}

void UdpSocketWin::SendMessage(ByteView data, const IPEndpoint& dest) {
    if (handle_.handle == INVALID_SOCKET) {
         if (client_) client_->OnSendError(this, Error::Code::kSocketClosedFailure);
         return;
    }
...
void UdpSocketWin::ReceiveMessage() {
    ...
    int bytes = recvfrom(handle_.handle, buffer, sizeof(buffer), 0, (sockaddr*)&src_addr, &src_len);
    ...
    if (src_addr.ss_family == AF_INET) {
         ...
         source_endpoint.address = IPAddress(IPAddress::Version::kV4, (uint8_t*)&sin->sin_addr);
    } else if (src_addr.ss_family == AF_INET6) {
         sockaddr_in6* sin6 = (sockaddr_in6*)&src_addr;
         source_endpoint.port = ntohs(sin6->sin6_port);
         source_endpoint.address = IPAddress(IPAddress::Version::kV6, (uint8_t*)&sin6->sin6_addr);
    }
    packet.set_source(source_endpoint);
    
    auto shared_packet = std::make_shared<UdpPacket>(std::move(packet));
    task_runner_.PostTask([weak_this = weak_factory_.GetWeakPtr(), shared_packet]() {
        if (auto* self = weak_this.get()) {
            if (self->client_) {

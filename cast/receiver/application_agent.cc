...
void ApplicationAgent::OnMessage(VirtualConnectionRouter* router,
  ...
  if (ns == kHeartbeatNamespace) {
    ...
    OSP_LOG_INFO << "[kReceiverNamespace] Received Message type="
                 << value.asString();

    OSP_LOG_INFO << json::Stringify(request.value()).value();
    ...
    const Json::Value& value =
        request.value().get(kMessageKeyType, Json::Value::nullSingleton());
    OSP_LOG_INFO << "[kSetupNamespace] Received Message type="
                 << value.asString();
    ...
    const Json::Value& value =
        request.value().get(kMessageKeyType, Json::Value::nullSingleton());
    OSP_LOG_INFO << "[kDiscoveryNamespace] Received Message type="
                 << value.asString();
...
Json::Value ApplicationAgent::HandleEurekaInfo(const Json::Value& request) {
  Json::Value response;
  OSP_LOG_INFO << "Processing EurekaInfo request";
...
Json::Value ApplicationAgent::HandleLaunch(const Json::Value& request,
  ...
  if (!error.ok()) {
    ...
    response[kMessageKeyType] = "LAUNCH_STATUS";
    response["launchRequestId"] = request[kMessageKeyRequestId];
    response["status"] = "USER_ALLOWED";
  }
  return response;
}
...
void ApplicationAgent::PopulateDeviceInfo(Json::Value* out) {
  ...
  message["controlNotifications"] = 1;
  message["deviceCapabilities"] = 268293;
  message["deviceId"] = receiver_info_.GetInstanceId();
  message["deviceModel"] = receiver_info_.model_name;
  message["friendlyName"] = receiver_info_.friendly_name;
  message[kMessageKeyType] = "DEVICE_INFO";
}
...
void ApplicationAgent::PopulateEurekaInfo(Json::Value* out) {
  ...
  data["version"] = 12;
  data["name"] = receiver_info_.friendly_name;

  Json::Value& device_info = data["device_info"];
  device_info["manufacturer"] = "google";
  device_info["product_name"] = receiver_info_.model_name;
  device_info["ssdp_udn"] = receiver_info_.GetInstanceId();

  Json::Value& build_info = data["build_info"];
  build_info["build_type"] = 2;
  build_info["cast_build_revision"] = "0.1";
  build_info["system_build_number"] = "BUILD_NUMBER";

  message["response_code"] = 200;
  message["response_string"] = "OK";
  message[kMessageKeyType] =
